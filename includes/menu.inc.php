...
class WasteSite {
    ...
    public static function getWasteSites() {
        ...
        foreach (self::$sites as $siteId => $siteDetails) {
            ...
            $output .= "<span class='badge bg-primary' style='font-size: 20px;'>Id: $siteId</span>";
            $output .= "<h6 class='card-title'>Kerület: {$siteDetails[0]}</h6>";
            $output .= "<h6 class='card-title'>Cím: {$siteDetails[1]}</h6>";

            $output .= "<div class='dropdown'>";
            $output .= "<button class='btn btn-primary dropdown-toggle' type='button' id='dropdownMenuButton_$siteId' data-bs-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>";
